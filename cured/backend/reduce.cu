...
void ragged_array_row_wise_sum_reduce(const int *data, const int ra_size, 
    const int *row_starts, const int *row_lengths, const int num_rows, int *ret, const bool on_device)
{
  int *data_d;
  int *row_starts_d;
  int *row_lengths_d;
  ...
  ragged_array_row_wise_sum_reduce_kernel<<<num_block, thread_block_size>>>(
      on_device ? data : data_d, 
      ra_size, 
      on_device? row_starts : row_starts_d, 
      on_device? row_lengths : row_lengths_d, 
      num_rows, 
      on_device? ret : ret_d
  );
